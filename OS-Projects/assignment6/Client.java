...
/**
 * Code for CSC 246 Assignment 6, Problem 1. Client supporting
...
public class Client {
  ...
  public static void main( String[] args ) {
    ...
    try {
      ...
      // Get the symmetric key from the server.
      byte[] encryptedSessionKey = Server.getMessage( input );
      byte[] sessionKey = RSADecrypter.doFinal( encryptedSessionKey );
      ...
      while ( scanner.hasNextLine() && ! ( request = scanner.nextLine() ).equals( "quit" ) ) {
        ...
        // Discard response if not to be printed.
        if ( !response.equals( "No Output" ))
