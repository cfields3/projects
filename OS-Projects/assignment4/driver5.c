...
void *cThread( void *arg ) {
  ...
  sleep( 2 );   // Then, wait 1 second before releasing it.
...
int main( int argc, char *argv[] ) {
  ...
  // Make a few threads
  pthread_t thread[ 3 ];
  if ( pthread_create( thread + 0, NULL, aThread, NULL ) != 0 ||
       pthread_create( thread + 1, NULL, bThread, NULL ) != 0 ||
       pthread_create( thread + 2, NULL, cThread, NULL ) != 0 )
